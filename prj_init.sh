...
while getopts "aut" opt; do
  case $opt in
    a)
      sed -i "s|#ADMINMOCKDATA|- ./SQL_Scripts/admin_mock_data.sql:/docker-entrypoint-initdb.d/04.sql|" compose.yaml 
      ;;
    u)
      sed -i "s|#USERMOCKDATA|- ./SQL_Scripts/uzivatel_mock_data.sql:/docker-entrypoint-initdb.d/03.sql|" compose.yaml 
      ;;
    t)
