...
def test_not_own():
    response = requests.get(f'{HOST}/api.php?service=delete_article&id=5', 
                            headers={'Authorization': get_id()})
    assert response.status_code == 403
    assert  "You can only delete your own articles" in response.json()["message"]

def test_ok():
    response = requests.get(f'{HOST}/api.php?service=delete_article&id=7', 
                            headers={'Authorization': get_id("rmar")})
    assert response.status_code == 200
    assert response.text == "OK"
    assert not os.path.exists("../Articles/test7.pdf")
    response = requests.get(f'{HOST}/api.php?service=get_user_articles&id=7', 
                            headers={'Authorization': get_id()})
    assert not article_id_in_list(response.json(),7)